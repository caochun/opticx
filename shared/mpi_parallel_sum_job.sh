...
#SBATCH --output=/shared/mpi_parallel_sum_%j.out
#SBATCH --error=/shared/mpi_parallel_sum_%j.err
...
# 编译MPI程序
echo "=== 编译MPI并行求和程序 ==="
mpicc -O2 /shared/mpi_parallel_sum.c -o /shared/mpi_parallel_sum
if [ $? -ne 0 ]; then
    echo "编译失败！"
    exit 1
fi
...
# 运行MPI程序
echo "--- 开始MPI并行求和计算 ---"
mpirun -n $SLURM_NTASKS --hostfile /tmp/hostfile /shared/./mpi_parallel_sum
